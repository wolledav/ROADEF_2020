...
Solution::Solution(Instance *instance, std::default_random_engine *engine) {
    ...
    this->final_objective = numeric_limits<fitness_t>::max();
...
void Solution::schedule(uint_t intervention_id, uint_t start_time) {
...
void Solution::unschedule(uint_t intervention_id) {
    if (!this->is_scheduled(intervention_id)) {
...
bool Solution::is_scheduled(uint_t intervention_id) {
    return this->scheduled.find(intervention_id) != this->scheduled.end();
}

uint_t Solution::get_start_time(uint_t intervention_id) {
...
uint_t Solution::get_unscheduled_count() {
    return (uint_t)this->unscheduled.size();
}

bool Solution::has_unscheduled() {
    return !(this->unscheduled.empty());
}

uint_t Solution::get_first_unscheduled() {
    return *(this->unscheduled.begin());
}

void Solution::save(string output_file_path) {
    std::ofstream output;
...
Objective Solution::estimate_schedule(uint_t intervention_id, uint_t start_time) {
    if (this->is_scheduled(intervention_id)) {
...
Objective Solution::estimate_unschedule(uint_t intervention_id) {
    if (!this->is_scheduled(intervention_id)) {
...
void Solution::update_state_on_schedule(uint_t scheduled_intervention, uint_t start_time) {
    uint_t scenarios;
    uint_t horizons = this->instance->get_horizon_num();
    fitness_t risk_st;
    fitness_t risk_t_;
    ...
    fitness_t excess;
    uint_t Qtt_index;
    array<fitness_t, ARR_SIZE> arr;
    vector<fitness_t> vec;
...
void Solution::update_extended_state_on_schedule(uint scheduled_intervention, uint_t start_time) {
    fitness_t before;
    fitness_t after;
    fitness_t gain;
    fitness_t max;
...
void Solution::update_state_on_unschedule(uint_t unscheduled_intervention, uint_t start_time) {
    uint_t scenarios;
    uint_t horizons = this->instance->get_horizon_num();
    fitness_t risk_st;
    fitness_t risk_t_;
...
void Solution::update_extended_state_on_unschedule(uint unscheduled_intervention, uint_t start_time) {
    fitness_t before;
    fitness_t after;
    fitness_t loss;
    fitness_t max;
...
void Solution::estimate_state_on_schedule(uint_t scheduled_intervention, uint_t start_time, Objective *objective) {
    fitness_t mean_risk = this->mean_risk;
    fitness_t expected_excess = this->expected_excess;
    fitness_t risk_st;
    fitness_t risk_t_;
    uint_t scenarios;
    uint_t horizons = this->instance->get_horizon_num();
    ...
    array<fitness_t, ARR_SIZE> arr;
    vector<fitness_t> vec;
    fitness_t mean_cumulative_risk;
    uint_t risk_in_s_idx = this->instance->get_risk_index(scheduled_intervention, start_time, start_time);
    half_fitness_vector_t risk_in_s;
    for (uint_t t = start_time; t < start_time + this->instance->delta[this->instance->get_delta_index(scheduled_intervention, start_time)]; ++t) {
        risk_in_s = this->instance->risk[risk_in_s_idx++];
...
void Solution::estimate_extended_state_on_schedule(uint scheduled_intervention, uint_t start_time, Objective *objective) {
    fitness_t before;
    fitness_t after;
    fitness_t gain;
    fitness_t max;
    fitness_t min;
    fitness_t workload_overuse = this->workload_overuse;
    fitness_t workload_underuse = this->workload_underuse;
    fitness_t total_resource_use = this->total_resource_use;
...
void Solution::estimate_state_on_unschedule(uint_t unscheduled_intervention, uint_t start_time, Objective *objective) {
    fitness_t mean_risk = this->mean_risk;
    fitness_t expected_excess = this->expected_excess;
    fitness_t risk_st;
    fitness_t risk_t_;
...
void Solution::estimate_extended_state_on_unschedule(uint unscheduled_intervention, uint_t start_time, Objective *objective) {
    fitness_t before;
    fitness_t after;
    fitness_t loss;
    fitness_t max;
...
void Solution::print_objective() {
    printf("%f\n", this->final_objective);
}

void Solution::print_extended_objective() {
    printf("%f\n", this->extended_objective);
}
...
void Solution::print_unscheduled_cnt() {
    ...
    void Solution::throw_message(const bool error, const string &label, const string &message) {}
    string Solution::time_bounds_details(const string &item, const uint_t start_time, const uint_t bound) { return ""; }
    string Solution::intervention_time_bounds_details(const string &item, const uint_t start_time, const uint_t bound) { return ""; }
    string Solution::workload_bounds_details(const bool upper, const string &item, const uint_t horizon, const fitness_t value, const uint_t bound) { return ""; }
    string Solution::exclusion_details(const string &first, const string &second, const uint_t horizon) { return ""; }

#else

    void Solution::throw_message(const bool correct, const string &label, const string &message) {
        cerr << (correct ? "#  >> Test passed <<" : "!  >> Error in solution <<") << endl;
        cerr << "\t" << label << " - " << message << endl;
    }

    string Solution::time_bounds_details(const string &item, const uint_t start_time, const uint_t bound) {
        ostringstream msg;
        msg << "Intervention '" << item << "' starts at time horizon " << start_time << " but planification range ends in " << bound << endl;
        return msg.str();
    }

    string Solution::intervention_time_bounds_details(const string &item, const uint_t start_time, const uint_t bound) {
        ostringstream msg;
        msg << "Intervention '" << item << "' starts at time horizon " << start_time << " but cannot start after " << bound << endl;
        return msg.str();
    }

    string Solution::workload_bounds_details(const bool upper, const string &item, const uint_t horizon, const fitness_t value, const uint_t bound) {
        ostringstream msg;
    ...
    string Solution::exclusion_details(const string &first, const string &second, const uint_t horizon) {
        ostringstream msg;
...
bool Solution::undefined_intervention() {
    bool ret = CORRECT;
    for (uint_to_uint_t::iterator iter = this->start_times.begin(); iter != this->start_times.end(); ++iter) {
...
bool Solution::unscheduled_intervention() {
    bool ret = CORRECT;
...
bool Solution::start_time_out_of_range() {
    bool ret = CORRECT;
    for (uint_to_uint_t::iterator iter = this->start_times.begin(); iter != this->start_times.end(); ++iter) {
...
bool Solution::start_time_out_of_intervention_range() {
    bool ret = CORRECT;
    for (uint_to_uint_t::iterator iter = this->start_times.begin(); iter != this->start_times.end(); ++iter) {
...
bool Solution::resource_out_of_bounds() {
    bool ret = CORRECT;
    fitness_t  workload;
...
void Solution::compute_objectives() {
    fitness_t mean_risk = 0.0; /* sum of mean risks in all times */
    fitness_t mean_risk_in_time; /* help variable to store mean risk in current time */
    fitness_t temp; /* help variable to hold values which are used on multiple places to avoid computing the same thing */
    vector<fitness_t> quantile_in_time; /* holds risks in time to compute quantile */
    quantile_in_time.resize(this->instance->get_max_scenarios());
    fitness_t excess; /* variable to hold excess in time t max(0, q_t - mean_risk_t) */
    fitness_t total_excess = 0.0; /* sum of quantiles in all times */
    ...
    fitness_t q; /* help variable to hold max(0, q_t - mean_risk_t) */
...
void Solution::compute_final_objective() {
    this->final_objective = this->instance->get_alpha() * this->mean_risk + (1 - this->instance->get_alpha()) * this->expected_excess;
}

void Solution::compute_workload_misuse() {
    fitness_t workload_ct;
    ...
    for (uint_t resource : this->instance->get_resources()) {
        ...
        for (uint_t time : this->instance->get_horizons()) {
            ...
            fitness_t max = this->instance->u[this->instance->get_ul_index(resource, time)];
            fitness_t min = this->instance->l[this->instance->get_ul_index(resource, time)];
            if (workload_ct > max + NUMERIC_TOLERANCE) {
                ...
                fitness_t tmp = ((workload_ct - min) / (max - min));
...
void Solution::count_unmet_exclusions() {
...
void Solution::compute_extended_objective() {
    this->extended_objective = this->final_objective + BETA_LOWER * this->workload_underuse + BETA_UPPER  * this->workload_overuse + GAMMA * this->exclusion_penalty;
}

void Solution::generate_random_solution() {
...
void Solution::round_up(int digits) {
